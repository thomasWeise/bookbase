...
For example, in an~\emph{instance} of the \pgls{optTSP} could define the number~\scale\ of cities and their distances. %
If our solutions are permutations of cities, then \scale~is also the dimension of~\solutionSpace. %
...
%
%
\newSymbol{scale}{\ensuremath{n}}{n}{%
We usually use~\scale\ to denote the scale, i.e., the size of the problem to solve. %
For example, %
in a \gls{optTSP}, this may be the number of cities, %
in a discrete benchmark problem such as \gls{optOneMax} or \gls{optTrap}, this may be the number of bits over which the problem is defined, %
in a \gls{optQAP}, this may be the number of facilities and locations, and %
in a \gls{optMaxSAT}~problem, this may be the number of variables that are input to the Boolean formula that we want to become true. %
There are also problems where the scale is not defined straightforwardly like this. %
For example, in a \gls{optJSSP}, we have a number of machines and a number of jobs, which together define the size of the problem. %
But even then, the scale~\scale\ relates to the size of the solution space~\solutionSpace, and, hence, also of the search space~\searchSpace, which usually grow exponentially with~\scale. %
And this is already the main issue: %
The larger~\scale, the (exponentially) larger the number of possible solutions and thus, the larger the runtime of algorithms.%
}%
%
%
...
On many benchmark problems, the objective functions~\objFun\ directly evaluate the elements~$\sespel\in\searchSpace$, i.e., are defined as~$\objFun:\searchSpace\mapsto\realNumbers$. %
However, very often these elements~\sespel\ are not the same data structures that the user is interested in and/or that the objective functions evaluate: %
It is common that the search space~\searchSpace\ is a simple and plain data structure that can be tackled with standard operations or mathematical tools, such as real vectors~$\realNumbers^n$, permutations, or bit strings, whereas the solution space~\solutionSpace\ can be something more complicated, like a Gantt chart. %
