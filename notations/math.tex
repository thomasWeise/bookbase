...
\mbox{If $f(x)=\bigOb{g(x)}$,} then there exist positive numbers~$x_0\in\realNumbersP$ and~$c\in\realNumbersP$ such that~\mbox{$0\leq f(x)\leq c*g(x)\forall x\geq x_0$}~\cite{B1894DAZDVPB,L1909HDLVDVDP,K1976BOABOABT,K1997FA}. %
In other words, \bigOb{g(x)}~describes an upper bound for function growth}%
%
\newFunc{bigOmega}{\ensuremath{\mathbf{\Omega}}}{O(g(x))}{g(x)}{%
\mbox{If $f(x)=\bigOmegab{g(x)}$,} then there exist positive numbers~$x_0\in\realNumbersP$ and~$c\in\realNumbersP$ such that~\mbox{$f(x)\geq c*g(x)\geq 0\forall x\geq x_0$}~\cite{K1976BOABOABT,K1997FA}. %
In other words, \bigOmegab{g(x)}~describes a lower bound for function growth}%
%
\newFunc{bigTheta}{\ensuremath{\mathbf{\Theta}}}{O(g(x))}{g(x)}{%
\mbox{If $f(x)=\bigThetab{g(x)}$,} then $f(x)=\bigOb{g(x)}$ and $f(x)=\bigOmegab{g(x)}$~\cite{K1976BOABOABT,K1997FA}. %
In other words, \bigThetab{g(x)}~describes an exact order of function growth}%
%
%
%
\protected\gdef\complexConj#1{\ensuremath{\overline{#1}}\glslink{complexConj}{\strut}}%
\newglossaryentry{complexConj}{%
type={symbols},
name={\complexConj{z}},%
text={\complexConj{z}},%
sort={z},%
description={\sloppy%
The conjugate~\complexConj{z} of a complex number~$z\in\complexNumbers$ with~$z=a+b\imaginaryUnit$ is~$\complexConj{z}=a-b\imaginaryUnit$~\cite{VDC2022AITCN}. %
You get the complex conjugate of a complex number by flipping the sign of the imaginary part.%
}}%
%
\newSymbol{complexNumbers}{\ensuremath{\mathSpace{C}}}{C}{%
the set of complex numbers is defined as~$\complexNumbers=\{(a,b): a,b\in\realNumbers\}$. %
Complex numbers~$z$ are often written in the standard form~$z=a+b\imaginaryUnit$, where \imaginaryUnit~is the so-called imaginary unit with~$\imaginaryUnit^2=-1$~\cite{VDC2022AITCN}.
}%
%
...
%
\newSymbol{imaginaryUnit}{\ensuremath{\mathbf{\mathrm{i}}}}{i}{%
The imaginary unit~\imaginaryUnit, which can also be written as~$\imaginaryUnit=(0,1)\in\complexNumbers$, is defined as~$\imaginaryUnit=\sqrt{-1}$ and~$\imaginaryUnit^2=-1$~\cite{VDC2022AITCN}%
}%
%
\newFunc{imaginaryPart}{\ensuremath{\operatorname{Im}}}{Im(z)}{z}{%
The \emph{imaginary part}~\imaginaryPartb{z} of a complex number~$z\in\complexNumbers$ with~$z=a+b\imaginaryUnit$ is~$\imaginaryPartb{z}=b$~\cite{VDC2022AITCN}%
}%
%
...
is the class of computational problems that can be solved in polynomial time by a non-deterministic machine and can be verified in polynomial time by a deterministic machine~(such as a normal computer)~\cite{G2022AGITNC}%
}%
%
\newSymbol{npComplete}{\@NPprefix\nobreakdashes-complete}{NP\nobreakdashes-complete}{%
...
%
\newSymbol{npHard}{\@NPprefix\nobreakdashes-hard}{NP\nobreakdashes-hard}{%
Algorithms that guarantee to find the correct solutions of \gls{classNP}\nobreakdashes-hard problems~\cite{LLRKS1993SASAAC,CPW1998AROMSCAAA,C1971TCOTPP} have a worst-case runtime that grows exponentially with the problem scale. %
...
In \postgresql, you can obtain it via the \sql\ function~\sqlil{exp(1)}\sqlIdx{exp} as value~\sqlilIdx{2.718281828459045}~\cite{PGDG:PD:MFAO}%
}%
%
%
...
In \postgresql, it is provided by the \sql\ function~\sqlil{pi()}\sqlIdx{pi} with value~\sqlilIdx{3.141592653589793}~\cite{PGDG:PD:MFAO}%
}%
%
%
...
%
%
\newFunc{realPart}{\ensuremath{\operatorname{Re}}}{Re(z)}{z}{%
The \emph{real part}~\realPartb{z} of a complex number~$z\in\complexNumbers$ with~$z=a+b\imaginaryUnit$ is~$\realPartb{z}=a$~\cite{VDC2022AITCN}%
}%
%
