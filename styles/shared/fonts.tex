%%
%% Do some font and internationalization setup.
%%
%
\RequirePackage[utf8]{inputenc}%
\RequirePackage[T1]{fontenc}%
%
\newcount\@bb@quote@depth\relax%
\@bb@quote@depth-1\relax%
%
%% Put text in quotes.
%% This command uses naturally double quotes, but if you nest quotations,
%% it will use single quotation marks, then double-triangle ones, and finally
%% just single triangular marks.
%% #1 the text
\protected\gdef\inQuotes#1{%
\advance\@bb@quote@depth by 1\relax%
\ifcase\@bb@quote@depth\iq@l@dquot\or`\or\guillemotleft\else\guilsinglleft\fi%
#1%
\ifcase\@bb@quote@depth\iq@r@dquot\or'\or\guillemotright\else\guilsinglright\fi%
\advance\@bb@quote@depth by -1\relax%
}%
%% The left and right double quote characters used in "inQuotes".
%% Can be overwritten in German language
\xdef\iq@l@dquot{``}%
\xdef\iq@r@dquot{''}%
%
%% Put text in single quotes.
%% #1 the text
\protected\gdef\inSQuotes#1{`#1'}%
%
%% Put text in guillemots.
%% #1 the text
\protected\gdef\inGuillemot#1{\guillemotleft#1\guillemotright}%
%
%% Render text in the style to render a software name
%% #1 the text
\ifIsBook\expandafter\@firstoftwo\else\expandafter\@secondoftwo\fi{%
\protected\gdef\softwareStyle#1{\scalebox{0.95}[1]{\texttt{#1}}}%
}{}%
\ifIsSlides\expandafter\@firstoftwo\else\expandafter\@secondoftwo\fi{%
\protected\gdef\softwareStyle#1{#1}%
}{}%
%
%% We use sans-serif as default font
\renewcommand{\familydefault}{\sfdefault}%%
%
%% Chinese character support
%%
\RequirePackage{CJKutf8}%
\AtBeginDocument{\CJK{UTF8}{gkai}}%
%\AtEndDocument{\end{CJK}}%
%
%% The check marks and x marks
\RequirePackage{pifont}%
\protected\gdef\greenYes{\textbf{\textcolor{greenYesColor}{\ding{51}}}}%
\protected\gdef\redNo{\textbf{\textcolor{redNoColor}{\ding{55}}}}%
%
%% Change the font size relative to the current size.
%% #1 the amount of points to change, can be positive (starting with +; untested) or negative (starting with -)
%% #2 the text to render
%% based on https://tex.stackexchange.com/questions/365024
\protected\gdef\changeFontSizeRel#1#2{%
\begingroup%
\fontsize{\dimexpr\f@size pt#1pt}{\f@baselineskip}\selectfont%
#2
\endgroup%
}%
%
