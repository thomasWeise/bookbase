...
%
%% The placement for listing boxes
\protected\gdef\listingFrame#1{%
\begin{figure}[tb]%
\centering%
#1%
\end{figure}%
...
\listingDoubleBox{#1}{%
#2~(stored in file~\programUrl{#1}; output in~\cref{exec:#1})%
}{\captionStdall{#1}}{,style=python_style}{,style=tool_style}{%
\expandafter\listingSepPython{\gitNameEsc{lst:#1}}%
}%
}%
%
...
%
%% Place a C listing with output.
%% #1 the label
%% #2 the caption
%% #3 the format of the output data
%%
\protected\gdef\listingCandOutput#1#2#3{%
\listingDoubleBox{#1}{%
#2~(stored in file~\programUrl{#1}; output in~\cref{exec:#1})%
}{\captionStdout{#1}}{,style=c_style}{#3}{%
\expandafter\listingSepC{\gitNameEsc{lst:#1}}%
}%
}%
