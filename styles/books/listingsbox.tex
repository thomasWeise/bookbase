...
%% #5 the script to invoke
%% #6 the label (lst: will be prepended to the program, exec: will be
%%    pre-pended to the output)
%% #7 the caption
\protected\gdef\gitSQLAndOutput#1#2#3#4#5#6#7{%
\expandafter\expandafter\expandafter\edef\expandafter\csname @gitSQLAndOutputFormat:exec:#6\endcsname{\@gitSQLAndOutputFormat}%
\gitSQLAndOutputFormatDefault%
%
\gitLoad{#1}{#2/#3}{}%
\expandafter\expandafter\expandafter\edef\expandafter\csname @pwp@gitUrl:lst:#6\endcsname{\gitUrl}%
\expandafter\expandafter\expandafter\edef\expandafter\csname @pwp@gitFile:lst:#6\endcsname{\gitFile}%
\gitExec{#1}{.}{_scripts_/#5 #2 #3 #4}%
\expandafter\expandafter\expandafter\edef\expandafter\csname @pwp@gitFile:exec:#6\endcsname{\gitFile}%
%
\begin{figure}[tb]%
\centering%
\lstinputlisting[label={lst:#6},style=sql_style,caption={#7~(stored in file~\href{\csname @pwp@gitUrl:lst:#6\endcsname}{\textil{#3}}; output in~\cref{exec:#6})}]{\csname @pwp@gitFile:lst:#6\endcsname}%
%
\medskip%
%
\edef\@@@@gitOutputFmt{\csname @gitSQLAndOutputFormat:exec:#6\endcsname}%
\lstinputlisting[label={exec:#6},style=\@@@@gitOutputFmt,caption={The \gls{stdout} resulting from the \sql\ statements in~\href{\csname @pwp@gitUrl:lst:#6\endcsname}{\textil{#3}} given in~\cref{lst:#6}.}]{\csname @pwp@gitFile:exec:#6\endcsname}%
\end{figure}%
}%
%
%
